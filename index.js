...
var upRight,upLeft,downLeft,downRight;  //all possible walking options for a king
...
var multiplier = 1 //2 if you make the jump 1 in case of contact
...
/*===============Initialization of fields of play ===========================================*/
...
function showMoves (piece) {
	/* if it was selected before a piece delete, reserved for the new piece selected
	...
	*/
	...
	selectedPiece = piece;
	var i,j; // retains the index of the king
	for ( j = 1; j <= 12; j++){
	...
	if(!match) {
	 return 0 ; // if no match has been found; it happens when for example red move and you press black
	}

	/*===now according to their color I set the edges and movements of the king===*/
	if(the_checker[i].color =="white"){
 	...
 	/*===========CHECK IF I CAN ATTACK====*/
		...
		attackMoves(the_checker[i]); // check if I have any movement of attack
...
function makeMove (index) {
	var isMove = false;
	if(!selectedPiece) // if the game of about started and no piece was selected
		return false;
 ...
 /* =========the perspective is of the player who moves ======*/
	...
	if(mustAttack)  // to know if I jump only one row or 2
		multiplier = 2;
	else
		multiplier = 1;
		...
		if(index == cpy_upRight){
			...
			if(the_checker[1].color=="white"){
				...
				//remove the piece if a bounce has been executed
...
/*===========MOVING THE PART-CHANGING COORDINATES======*/

function executeMove (X,Y,nSquare){
	// exchange coordinated part moved
	the_checker[selectedPieceindex].changeCoord(X,Y); 
	the_checker[selectedPieceindex].setCoord(0,0);
	// I release the field that the piece occupies and occupy the one it will occupy
